...
namespace Breifico.Algorithms.Sorting
{
    public class SelectionSorting<T> : ISorter<T> where T : IComparable<T>
    {
        public T[] Sort(T[] input) {
            for (int i = 0; i < input.Length - 1; i++) {
                ...
                var tmpValue = input[i];
                input[i] = input[minIndex];
                input[minIndex] = tmpValue;
            }
            return input;
        }
    }
    ...
    public class SelectionSortingTests
    {
        ...
        public void SelectionSorting_Sort_WhenMultipleElementsInArray_ShouldSort() {
            var input = new[] { 1, 2 };
            input.MySort(new SelectionSorting<int>()).Should().Equal(1, 2);

            var input2 = new[] { 2, 1 };
            input2.MySort(new SelectionSorting<int>()).Should().Equal(1, 2);

            var input3 = new[] {9, 5, 4, 3};
            input3.MySort(new SelectionSorting<int>()).Should().Equal(3, 4, 5, 9);

            var input4 = new[] {5, 9, 5, 9, 2, 1, 2, 6, 5, 4, 7};
            input4.MySort(new SelectionSorting<int>()).Should().Equal(1, 2, 2, 4, 5, 5, 5, 6, 7, 9, 9);

            var input5 = new[] {1, 2, 3, 4, 5, 6, 7};
            input5.MySort(new SelectionSorting<int>()).Should().Equal(1, 2, 3, 4, 5, 6, 7);
        }
    }
}